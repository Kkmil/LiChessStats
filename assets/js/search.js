...
function setColor(color) {
    selectedColor = color;
    fetchStats(document.getElementById("searchbar").value);
}
...
function search() {
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            // Fetch stats with the selected color
            fetchStats(username);
        });
}
function fetchStats(username, selectedColor) {
	var selectedColor = white;
    let url = `https://explorer.lichess.ovh/player?player=${username}&color=$(selectedColor);

    fetch(url)
        .then(response => response.json())
        .then(data => {
            if (selectedColor === 'white') {
                document.getElementById("white_games").innerText = data.white;
            } else if (selectedColor === 'black') {
                document.getElementById("black_games").innerText = data.black;
            }
        })
